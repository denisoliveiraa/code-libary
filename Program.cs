...
Library library = new Library();
 library.registerBook(new Book("O Senhor dos Anéis", "A Sociedade do Anel", "J.R.R. Tolkien", 1954, 1));
 library.registerBook(new Book("Harry Potter", "A Pedra Filosofal", "J.K. Rowling", 1997, 2));
 library.registerBook(new Book("Dom Quixote", "A Jornada do Cavaleiro", "Miguel de Cervantes", 1605, 3));
 library.registerBook(new Book("1984", "O Grande Irmão", "George Orwell", 1949, 4));
 library.registerBook(new Book("O Pequeno Príncipe", "Uma Jornada no Deserto", "Antoine de Saint-Exupéry", 1943, 5));

 library.registerClient(new Client("João", "Silva", 1980, library.CountBooks + 1));
library.registerClient(new Client("Maria", "Oliveira", 1992, library.CountBooks + 2));
library.registerClient(new Client("Pedro", "Santos", 1985, library.CountBooks + 3));
library.registerClient(new Client("Ana", "Costa", 1990, library.CountBooks + 4));
library.registerClient(new Client("Lucas", "Almeida", 2000, library.CountBooks + 5));
...
while (menuStarting)
{
    ...
    switch (res)
    {
        case "1":
            ...
            library.registerBook(new Book(booketitle, bookSubtitle, bookAuthor, BookReleaseYear, library.CountClient + 1));
